...
Func IsInstallerPageLoaded($_WinTitle, $_WinText ,$_RetryTimes = 500 )
	...
	While $IsButtonVisible < 1 Or $IsButtonEnabled < 1
		...
		EndIf
	WEnd
	FileWriteLine($LogFile, _Now() & ": INFO: The Installer Page:"& $_WinText &" successfully loaded after retry " & $Retry & "times, return True")
	Return True
EndFunc
...
Func Snapshot($CurrentWindow,$FileName,$WindowText="")

	If DirGetSize(@ScriptDir & "\" & "Snapshots\") == -1 Then
       DirCreate(@ScriptDir & "\" & "Snapshots\")
    EndIf


	WinActivate($CurrentWindow,$WindowText)
	Sleep($SNAPSHOT_WAIT_TIME)
	_ScreenCapture_Capture(@ScriptDir & "\" & "Snapshots\" & $FileName)
	Sleep($SNAPSHOT_WAIT_TIME)
EndFunc
