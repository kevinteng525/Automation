...
Local $ID = "OfflineAccess_32_"
Local $LogFile = FileOpen("C:\Scripts\OA_Logging.txt",1)
...
Local $Setup=0
Local $hSearch = FindFile($InstallerLocation, "ES1_OfflineAccess.exe",12)

If $hSearch == 0 Then
	FileWriteLine($LogFile,_Now() & ":INFO: ES1_OfflineAccess.exe is NOT found")
	MsgBox(64,"Warning!","ES1_OfflineAccess.exe is NOT found, exit.")
	Exit
ElseIf  $hSearch[0] == 1 Then
	$Setup = $hSearch[1]
	FileWriteLine($LogFile,_Now() & ":INFO: ES1_OfflineAccess.exe is found: " & $Setup )
	FlushLogging()
...
Func OAInstall()
   FileWriteLine($LogFile,_Now() & ":INFO: OfflineAccess Install operation has been started..")
   Local $btnFinishID
...
;~ 	$bLoadingComplete = IsMasterCompleteLoading($WinTitle,"&Next >")
   ...
   ClickButtonByName($WinTitle, "&Install")
	...
	FileWriteLine($LogFile, _Now() & "INFO: Finish button Clicked!")
	  ...
	  FileWriteLine($LogFile, _Now() & ":OA install done")

EndFunc

Func OARepair()
   FileWriteLine($LogFile,_Now() & ":INFO: OfflineAccess Repair operation has been started..")
   Local $btnFinishID
   Local $bLoadingComplete
...
;~ 	$bLoadingComplete = IsMasterCompleteLoading($WinTitle,"&Next >")
   ...
   FileWriteLine($LogFile, _Now() & ":OA Repair done")
EndFunc

Func OAUninstall()
   FileWriteLine($LogFile,_Now() & ":INFO: OfflineAccess Uninstall operation has been started..")
   Local $btnFinishID
   Local $bLoadingComplete
...
;~ 	$bLoadingComplete = IsMasterCompleteLoading($WinTitle,"&Next >")
   ...
   FileWriteLine($LogFile, _Now() & ":OA Remove done")
EndFunc

Func OAUpgrade()
   FileWriteLine($LogFile,_Now() & ":INFO: OfflineAccess Upgrade operation has been started..")
   Local $btnFinishID
   Local $bLoadingComplete
...
;~ 	$bLoadingComplete = IsMasterCompleteLoading($WinTitle,"&Next >")
   ...
   FileWriteLine($LogFile, _Now() & ":OA Upgrade done")
   EndFunc
