...
Local $Setup=0
Local $hSearch = FindFile($InstallerLocation, "ES1_WebServicesSetup.exe.exe",12)

If $hSearch == 0 Then
	FileWriteLine($LogFile,_Now() & ":INFO: ES1_WebServicesSetup.exe.exe is NOT found")
	MsgBox(64,"Warning!","ES1_WebServicesSetup.exe.exe is NOT found, exit.")
	Exit
ElseIf  $hSearch[0] == 1 Then
	$Setup = $hSearch[1]
	FileWriteLine($LogFile,_Now() & ":INFO: ES1_WebServicesSetup.exe.exe is found: " & $Setup )
	FlushLogging()
