...
class TestSuite < SaberTestBase
  ...
  def test_close_matter_webid_131   
    ...
    if @dmweb.allItems_result_page.matterstate($mattername).text.include? "Closed"
      puts "close the matter success!"
      assert(true,message="closed the matter success!") 
    else
      puts "close the matter failed!" 
      assert(true,message="closed the matter failed!") 
    end 
