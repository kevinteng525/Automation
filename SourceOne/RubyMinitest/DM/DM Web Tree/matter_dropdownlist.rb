...
class TestSuite < SaberTestBase
  ...
  #Test case #1: can see matter from drop down list in my matters
  def test_matter_dropdownlist_webid_1367  
   ...
   @dmweb.main_head_page.clickMatterLink
    ...
    @dmweb.main_head_page.clickMatterLink
    sleep(2)
    @dmweb.Mattertree_items.dropdown_list.click
    sleep(2)
    size=@dmweb.Mattertree_items.dropdown_list_context.count
    flag=1
    if (@dmweb.Mattertree_items.dropdown_list_context)[0].text<=>"All"
      flag=0
    end
    for i in 1..size-1
      if (@dmweb.Mattertree_items.dropdown_list_context)[i].text<=>$mattername+i.to_s
       flag=0
       
      end
    end
   if flag
      puts "can see all the matters from drop down list of My matters success!"
      assert(true,message="can see all the matters from drop down list of My matters success!")    
    else
      puts "can see all the matters from drop down list of My matters failed!" 
      assert(false,message="can see all the matters from drop down list of My matters failed")      
    end 
