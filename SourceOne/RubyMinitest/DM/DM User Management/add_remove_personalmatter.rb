...
class TestSuite < SaberTestBase
  ...
  def test_001_add_exist_user_webid_1654    
    ...
    if firstuserstatus.include? "No"
       ...
       if @dmweb.main_head_page.mattersummary
       ...
       @dmweb.btn_id_helper.get_Dlg_btnID("Confirmation","Yes")
       firstusernewstatus=@dmweb.main_administration_page.get_firstuser_personalmatter_status
       if firstusernewstatus.include? "No"
       puts "#{firstuser} remove personal matter success"
       else 
       puts "#{firstuser} remove personal matter failed"
       end
       @dmweb.main_head_page.clickMatterLink
       if !@dmweb.main_head_page.mattersummary
       puts "add the personal matter have  work correct"
       ...
       puts "the personal matter have not work correct,have a problem" 
       assert(false,message="the personal matter have not work correct,have a problem")      
       end    
   end   
  end   
