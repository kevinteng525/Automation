...
def test_firstuser_focus_webid_1663
    ...
    if @dmweb.btn_id_helper.getdlg_content(a).text.include? $username
      puts "first user #{$username} focus success!"
      assert(true,message="first user #{$username} focus success!")
      
    else
      puts "first user #{$username} focus failed!"
      assert(false,message="first user #{$username} focus failed!")
    end
 end
