...
class TestSuite < SaberTestBase
  ...
  def test_001_newSearch_Cancel_webid_1729
    ...
    @dmweb.new_search_page.click_Cancel
    ...
    if !@dmweb.search_summary_page.validSearchName($searchname)
      puts "Search is Canceled success!"
      assert(true,message="Search is Canceled success!")
    else  
      puts "Search is Canceled failed"  
      assert(false,message="Search is Canceled failed")
    end 
