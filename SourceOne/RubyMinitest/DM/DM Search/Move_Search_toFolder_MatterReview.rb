...
class TestSuite < SaberTestBase
  ...
  def self.test_order
   ...
   # Move_Search_toFolder_MatterReview.
  def test_Move_Search_toFolder_MatterReview_webid_1744
    ...
    @dmweb.matter_tree_page.expandMatterNode
    @dmweb.matter_tree_page.clickLastMatterReview
    if @dmweb.search_summary_page.validSearchFolder($foldername)
      puts "move the Search to folder success!"
      assert(true,message="move the Search to folder success!")
    else  
      puts "move the Search to folder failed!"  
      assert(false,message="move the Search to folder failed!")
    end 
   end
 end
