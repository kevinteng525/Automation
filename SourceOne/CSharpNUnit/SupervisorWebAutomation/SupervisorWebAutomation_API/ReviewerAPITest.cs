...
namespace SupervisorWebAutomation_API
{
    ...
    public class ReviewerAPITest : BaseTest
    {
        ...
        public void TestReviewer_Workflow()
        {
            ...
            MessageListFilter nullFilter = new MessageListFilter();
            ...
            IRestResponse<List<FlaggedMessage>> getFlaggedMessagesByPageResponse_nullSortParam = APIGetFlaggedMessagesByPage2("0-99", totalMessageCount);

            Assert.IsTrue(getFlaggedMessagesByPageResponse_nullSortParam.StatusCode == System.Net.HttpStatusCode.OK, "Get message list By Page status code is not correct");

            Assert.IsTrue(GetHeader(getFlaggedMessagesByPageResponse_nullSortParam, "Content-Range") == string.Format("items {0}/{1}", "0-99", totalMessageCount), "Content range of get message list by page is not correct");
